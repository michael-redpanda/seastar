...
public:
    ...
    void verify() {
        ...
        if (res != X509_V_OK) {
            ...
            auto dn = extract_dn_information(is_verification_error_t::yes);
            if (dn) {
...
private:
    ...
    using is_verification_error_t = bool_class<struct is_verification_error_tag>;

    std::optional<session_dn> extract_dn_information(is_verification_error_t verification_error = is_verification_error_t::no) const {
